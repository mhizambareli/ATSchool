...
public class Main {
    ...
    public static void snakePrint(int n, int col) {
        ...
        int j; //переменная для перебора всех чисел до n
        for (int i = 0; i < rows; i++) {
            if (i % 2 == 0) {
                for (j = i * col + 1; j <= (i + 1) * col && j <= n; j++) {
                    System.out.print(j + " ");
                }
            } else {
                for (j = (i + 1) * col; j > i * col && j <= n; j--) {
                    System.out.print(j + " ");
                }
            }
            //условия для корректного вывода последней строки, если она в обратном порядке
            if (i % 2 != 0 && i == (rows - 1) && j > n)
                for (j = n; j > i * col && j <= n; j--) System.out.print(j + " ");
